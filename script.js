...
function initializeEventListeners() {
  ...
  // Shape comparator inputs (optional)
  const shapeLengthInput = document.getElementById('shapeLength');
  const shapeWidthInput = document.getElementById('shapeWidth');

  if (shapeLengthInput && shapeWidthInput) {
    const updateShapeArea = () => {
      const length = parseFloat(shapeLengthInput.value);
      const width = parseFloat(shapeWidthInput.value);

      if (!isNaN(length) && !isNaN(width) && length > 0 && width > 0) {
        AppState.shapeLength = length;
        AppState.shapeWidth = width;
        AppState.shapeArea = length * width;
    ...
    shapeLengthInput.addEventListener('input', updateShapeArea);
    shapeWidthInput.addEventListener('input', updateShapeArea);
  }
...
function detectCoin(screenX, screenY) {
  ...
  try {
    ...
    if (detectedCircle) {
      ...
      // Calculate coin pixel and physical area
      AppState.coinPixelArea = Math.PI * detectedCircle.radius * detectedCircle.radius;
      const coinRadiusCm = AppState.referenceDiameter / 2;
      AppState.coinAreaCm2 = Math.PI * coinRadiusCm * coinRadiusCm;
...
function detectCoinHough(src, clickX, clickY) {
  try {
    ...
    for (let i = 0; i < circles.cols; i++) {
      ...
      // Accept circle only if click is precisely within the coin
      // Much tighter acceptance radius for high precision
      const acceptanceRadius = Math.max(radius * 0.6, 10);
  ...
  try {
    ...
    for (let i = 0; i < contours.size(); i++) {
      ...
      if (circularity > 0.6) {
        ...
        // Tighter, more precise click acceptance
        const acceptanceRadius = Math.max(radius * 0.6, 10);
        const isInsideCircle = distance <= radius;
        if (isInsideCircle && distance < acceptanceRadius && distance < minDistance) {
          minDistance = distance;
...
function drawCoinHighlightOnOutputCanvas() {
  ...
  try {
    ...
    // Create a mask for the coin circle
    let mask = new cv.Mat.zeros(src.rows, src.cols, cv.CV_8UC1);
    const center = new cv.Point(coin.x, coin.y);
    cv.circle(mask, center, coin.radius, new cv.Scalar(255), -1);

    // Apply a soft colored overlay (similar idea to drape highlight)
    for (let y = 0; y < src.rows; y++) {
      for (let x = 0; x < src.cols; x++) {
        if (mask.ucharAt(y, x) === 255) {
          const idx = (y * src.cols + x) * 4;
          // Blend a cyan overlay with original color
          src.data[idx] = Math.round(src.data[idx] * 0.5 + 0 * 0.5); // R
          src.data[idx + 1] = Math.round(src.data[idx + 1] * 0.5 + 255 * 0.5); // G
          src.data[idx + 2] = Math.round(src.data[idx + 2] * 0.5 + 255 * 0.5); // B
        }
      }
    }

    // Draw circle outline on top for clarity
    cv.circle(
      src,
      center,
      coin.radius,
      new cv.Scalar(0, 200, 200, 255),
      3
    );
  ...
  setTimeout(() => {
    try {
      ...
      if (bestContourIndex >= 0) {
        ...
        AppState.drapePixelArea = pixelArea;
        const actualAreaCm2 =
        ...
        if (pixelAreaEl) {
          pixelAreaEl.textContent = pixelArea.toFixed(0);
        }
