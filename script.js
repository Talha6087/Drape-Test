...
function updateCoinAreaDisplay() {
  ...
  // Physical coin area is based on the selected/reference diameter
...
function updateTargetAreaDisplays() {
  const target =
    (document.getElementById('refTarget') &&
      document.getElementById('refTarget').value) ||
    'coin';

  const coinAreaEl = document.getElementById('coinAreaCm2');
  const shapeAreaEl = document.getElementById('shapeAreaValue');

  // Shape (drape) area is always AppState.drapeArea (cm²)
  const shapeAreaCm2 =
    AppState.drapeArea && AppState.drapeArea > 0 ? AppState.drapeArea : 0;

  // Coin area from reference diameter
  const coinAreaCm2 =
    AppState.coinAreaCm2 && AppState.coinAreaCm2 > 0
      ? AppState.coinAreaCm2
      : 0;

  if (target === 'coin') {
    // Show coin area in the reference panel
    if (coinAreaEl) {
      coinAreaEl.textContent = coinAreaCm2
        ? coinAreaCm2.toFixed(2)
        : '--';
    }

    // Shape section continues to show detected drape area when available
    if (shapeAreaEl) {
      shapeAreaEl.textContent = shapeAreaCm2
        ? shapeAreaCm2.toFixed(2)
        : '--';
    }
  } else {
    // Target is shape (drape): show drape area in BOTH places
    if (coinAreaEl) {
      coinAreaEl.textContent = shapeAreaCm2
        ? shapeAreaCm2.toFixed(2)
        : '--';
    }
    if (shapeAreaEl) {
