  ...
  setTimeout(() => {
    try {
      ...
      if (bestContourIndex >= 0) {
        ...
        // The contour area represents the total shadow (support disk + draped fabric)
        const totalShadowPixelArea = bestContourArea;
        AppState.drapePixelAreaTotal = totalShadowPixelArea;

        // Support disk area in pixels (use current diskDiameter, default 18 cm)
        const diskRadiusPx =
          (AppState.diskDiameter / 2) * AppState.scaleFactor;
        const diskPixelArea = Math.PI * diskRadiusPx * diskRadiusPx;

        // Fabric-only shadow area in pixels (for display)
        let fabricShadowPixelArea = totalShadowPixelArea - diskPixelArea;
        if (fabricShadowPixelArea < 0) {
          console.warn(
            'Fabric shadow pixel area computed as negative; clamping to 0. Values:',
            { totalShadowPixelArea, diskPixelArea }
          );
          fabricShadowPixelArea = 0;
        }
        ...
        console.log(
          'Disk pixel area (support disk):',
          diskPixelArea.toFixed(0),
          'px²'
        );
        console.log(
