...
function detectCoinHough(src, clickX, clickY) {
  try {
    ...
    for (let i = 0; i < circles.cols; i++) {
      ...
      // Accept circle only if click is reasonably close to its center
      // Tighter acceptance radius for more precise selection (especially on mobile)
      const acceptanceRadius = Math.max(radius * 1.1, 20);
