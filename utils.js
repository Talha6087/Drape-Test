...
const DrapeFormulas = {
  ...
  /**
   * Calculate Drape Coefficient using the user-specified formula:
   * DC (%) = (Area of Fabric / Area of Shadow) × 100
   *
   * Where:
   * - Area of Fabric = π × (fabricDiameter/2)²
   * - Area of Shadow = projected fabric shadow area (green area only)
   *
   * Note: disk diameter is accepted for signature compatibility but
   * not used in this simplified formula.
   */
  drapeCoefficient: function (shadowArea, diskDiameter, fabricDiameter) {
    const fabricArea = this.circleArea(fabricDiameter);

    console.log('Drape Calculation:', {
      shadowArea: shadowArea.toFixed(2) + ' cm²',
      fabricDiameter: fabricDiameter + ' cm',
      fabricArea: fabricArea.toFixed(2) + ' cm²',
    });

    if (shadowArea <= 0) {
      console.warn('Shadow area must be positive');
      return 0;
    }

    const coefficient = (fabricArea / shadowArea) * 100;
