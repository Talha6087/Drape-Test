...
const DrapeFormulas = {
  ...
  /**
   * Calculate Drape Coefficient using Cusick formula:
   * DC (%) = ((Ad - As) / (Af - As)) × 100
   *
   * Where:
   * - Ad = Draped area (projected area of fabric shadow)
   * - As = Support disk area (π × (diskDiameter/2)²)
   * - Af = Fabric sample area (π × (fabricDiameter/2)²)
   *
   * Result ranges from 0% (stiff, no drape) to 100% (maximum drape)
   */
  drapeCoefficient: function (drapedArea, diskDiameter, fabricDiameter) {
    ...
    console.log('Drape Calculation:', {
      drapedArea: drapedArea.toFixed(2) + ' cm²',
    ...
    if (fabricArea <= diskArea) {
      console.warn('Fabric area must be greater than disk area');
      return 0;
    }

    const coefficient =
      ((drapedArea - diskArea) / (fabricArea - diskArea)) * 100;
